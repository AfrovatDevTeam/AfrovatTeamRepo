...
CREATE TRIGGER trg_AllowOverdraft
ON Transactions3
AFTER INSERT
AS
BEGIN
    ...
    BEGIN
        ...
        RAISERROR (
            'Transaction denied: Overdraft limit of R5000 exceeded.',
            16,
            1
        );
    END
...
---Bonus 2 Create a Reversal transactions table---

CREATE TABLE ReversalTransactions
(
    ReversalID INT IDENTITY PRIMARY KEY,
    TransactionID INT,
    ReversalDate DATETIME DEFAULT GETDATE(),
    Reason VARCHAR(255)
);

---BONUS 3 Trigger to update Clients.AccountBalance ---

CREATE TRIGGER trg_UpdateClientBalance
ON Accounts3
AFTER UPDATE
AS
BEGIN
    UPDATE c
    SET c.AccountBalance = (
        SELECT SUM(Balance)
        FROM Accounts3
...
---Bonus 4 Trigger to audit UPDATE and DELETE attempts ---
